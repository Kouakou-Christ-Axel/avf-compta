...
fn map_row(row: &Row) -> rusqlite::Result<Paiement> {
    Ok(Paiement {
        ...
        montant_cents: row.get("montant_cents")?,
...
pub fn insert(
    ...
    conn.execute(
        "INSERT INTO paiements (note_id, montant_cents, date_paiement, methode, cree_le)
         VALUES (?1, ?2, ?3, ?4, ?5)",
        rusqlite::params![note_id, montant_cents, date_paiement, methode, super::now()],
    )?;
...
/// Total déjà encaissé pour une note (centimes).
pub fn total_paye_cents(conn: &Connection, note_id: i64) -> AppResult<i64> {
    let total: i64 = conn.query_row(
        "SELECT COALESCE(SUM(montant_cents), 0) FROM paiements WHERE note_id = ?1",
        [note_id],
        |r| r.get(0),
    )?;
    Ok(total)
